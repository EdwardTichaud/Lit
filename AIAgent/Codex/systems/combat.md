...
Une `ThresholdSequence` lance un clip InPlace de Lucian contenant un ou plusieurs Animation Events `QTE(string input)`, dans leur ordre auteur. Au runtime, un `AnimatorOverrideController` temporaire injecte le clip QTE choisi dans l'etat generique `ThresholdSequence_QTE` et le clip de reussite choisi dans `Threshold_Succes`; le controller original est restaure a la fin de la sequence. Sans clip QTE specifique, `ThresholdSequence_QTE` conserve son clip par defaut. Chaque reussite ferme son overlay, laisse le clip continuer vers l'evenement suivant et n'active l'animation de succes qu'apres le dernier QTE valide. Une mort de palier est appliquee au delai configure, mais le combat reste suspendu jusqu'a la fin du clip de succes; le joueur fond ensuite vers `CombatIdle` avec `successExitBlendSeconds` avant la restitution. Un echec coupe la chaine par un fondu vers `CombatIdle`, puis applique le resultat d'echec de la sequence. Les valeurs sont `Y`, `B`, `A` et `X`; l'evenement ouvre la map isolee `CombatQTE` pendant la duree reelle configuree dans `CombatHealthThresholdController` (0,5 s par defaut; `Y/B/A/X` manette, fleches haut/droite/bas/gauche clavier). Pendant cette fenetre, `CombatLocalTimeField` ouvre autour du joueur concerne une sphere de 10 m a `0,4`: Lucian et tout `CombatTimeDomain` qui y entre ralentissent localement. Le temps global et les autres clients restent intacts. Une touche tenue avant l'ouverture est ignoree.
