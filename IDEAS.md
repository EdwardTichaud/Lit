...
- Attribution des joueurs : `NetcodePlayerSpawner` (serveur) assigne un personnage par client en utilisant `SquadManager.currentSquad` (fallback `Resources.FindObjectsOfTypeAll<CharacterData>()`), choisit un spawn via `squadSpawnPoints` ou `squadSpawnOrigin`, puis spawn un player Netcode par client. Il spawn aussi `WorldInteractionService` une seule fois.
