// Role:
// Squad effect that defines the maximum flame duration granted by a flame building/item.
// Usage:
// Assigned to flame-related ScriptableObject effect assets.
// Responsibilities:
// Provide per-level flame max duration text and values for other systems.
// Dependencies:
// Effect, ISquadEffect, SquadCharacterController.
// Precautions:
// This effect reports data; the actual flame reset logic lives elsewhere.
using System.Collections.Generic;
using UnityEngine;

/// <summary>
/// Defines max flame seconds for a squad-level flame effect.
/// </summary>
[CreateAssetMenu(fileName = "FlameEffect", menuName = "Scriptable Objects/Effects/Flame")]
public class FlameEffect : Effect, ISquadEffect
{
    [Header("Flame Max")]
    /// <summary>Maximum flame seconds at level 1.</summary>
    [Tooltip("Secondes max de flamme au niveau 1.")]
    ...
    /// <summary>
    /// Resolves max flame seconds for a 1-based level.
    /// </summary>
    ...
    public override string GetDescriptionForLevel(int level)
    {
        ...
        return $"Flamme max: {value}s (reset maison)";
    }
