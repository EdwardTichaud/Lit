...
public class Torch : NetworkBehaviour, ICharacterDetectedInteractable
{
    [Header("State")]
    [SerializeField, Tooltip("Etat de la torche au demarrage.")]
    private bool isLit = false;
    [SerializeField, Tooltip("Identifiant unique utilise pour la sauvegarde ou le debug.")]
    ...
    [SerializeField, Tooltip("Configure cette torche comme source de reveal/dissolve de monde.")]
    ...
    [Header("Interaction")]
    [SerializeField, Tooltip("Ecoute Interact en plus de TriggerMunin.")]
    private bool useInteractInput = false;
    [SerializeField, Tooltip("Ecoute TriggerMunin quand la torche est ciblee par le personnage local.")]
    ...
    private int interactionPriority = 70;
    [Tooltip("Rayon du trigger d'interaction.")]
    public float interactionRadius = 1.75f;
    ...
    public float emissionFadeDuration = 0.6f;

    [Header("Munin")]
    [SerializeField, Tooltip("Controleur de Munin. Laisse vide pour utiliser celui du personnage detecte.")]
    private MuninController muninController;
    [SerializeField, Tooltip("Offset local depuis l'ancre de la torche pour la destination de Munin.")]
    ...
    public bool TryResolvePersistentObjectId(out string id)
    {
        ...
        {
            id = $"scene-torch:{gameObject.scene.name}:{NetcodeSceneIdUtility.GetStableId(transform):X8}";
            return true;
        }

        id = $"runtime-torch:{name}:{GetInstanceID()}";
        return true;
    }
    ...
    private void EnsureRevealSource()
    {
        ...
        {
            torchLightReceiver = GetComponentInChildren<TorchLightReceiver>(true);
        }

        if (torchLightReceiver != null)
        {
            torchLightReceiver.ConfigureWorldRevealSource(true);
        }
    }
    ...
    private void ApplyLitActivationTargets()
    {
        ...
        {
            GameObject target = activateWhenLitTargets[i];
            if (target != null)
            {
    ...
    private void CollectFlameParticleSystems()
    {
        ...
        if (flameObject != null)
        {
            ParticleSystem[] systems = flameObject.GetComponentsInChildren<ParticleSystem>(true);
    ...
    {
        if ((!useInteractInput && !showStateDialogueOnInteract)
            || LocalInputRouter.IsInteractConsumed
    ...
    {
        if (!useTriggerMuninInput || InputFocusStack.HasAnyFocus())
        {
            return;
        }
    ...
    private GameObject GetSquadCharacter(Collider other)
    {
        ...
        return hasPlayerTag ? taggedPlayerRoot : null;
    }
    ...
    {
        if (character == null || !characterColliderCounts.TryGetValue(character, out int count))
        {
            return false;
        }
    ...
    {
        if (interactionInProgress || interactionRoutine != null)
        {
            return;
        }
    ...
    {
        EnsureLitInfluence();
        litInfluence.Tick(this, LitInfluenceSourceKind.Torch, isLit, force);
    }
    ...
    private void ClearLitInfluence()
    {
        ...
        {
            litInfluence.Clear(this, LitInfluenceSourceKind.Torch);
        }
    }
    ...
    private void EnsureLitInfluence()
    {
        if (litInfluence == null)
        {
            litInfluence = new LitInfluenceSource(4f);
        }
    }
