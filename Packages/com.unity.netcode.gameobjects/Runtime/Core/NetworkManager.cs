...
#pragma warning restore IDE0001
        ...
        {
            // Unity invokes OnDestroy after a completed Shutdown and also for a
            // NetworkManager that was never started. The remaining shutdown
            // path assumes live Spawn/Scene services and is not idempotent in
            // that state, which otherwise produces teardown exceptions.
            if (!IsListening && SpawnManager == null && SceneManager == null)
            {
                return;
            }
#if UNITY_EDITOR
